...
from pygpt_net.controller.config.field.input import Input


def test_apply(mock_window):
    """Test apply"""
    field = Input(mock_window)
    ...
    mock_window.ui.config['parent_id']['key'].setText.assert_called_once_with("123")
...
def test_get_value(mock_window):
    ...
    input = MagicMock()
    input.text = MagicMock(return_value="123")
    mock_window.ui.config = {
        'parent_id': {
            'key': input
        }
