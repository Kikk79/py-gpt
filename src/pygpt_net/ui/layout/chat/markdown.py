...
class MarkdownHighlighter(QSyntaxHighlighter):
    MARKDOWN_KEYS_REGEX = {
        ...
        'Header': re.compile(u'^(.+)[ \t]*\n(=+|-+)[ \t]*\n+'),
        'CodeBlock': re.compile(r'^ *(`{3,})(.*?)(`{3,})', re.DOTALL | re.MULTILINE),
    ...
    def __init__(self, parent):
        ...
        self.defaultTheme = {"background-color": "#d7d7d7", "color": "#fff",
    ...
    def setTheme(self, theme):
        ...
        format = QTextCharFormat()
        format.setBackground(QBrush(QColor(theme['codeblock']['background'])))
